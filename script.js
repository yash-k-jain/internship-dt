...
$(document).ready(function () {
  ...
  const populateData = () => {
    ...
    $(".journey-body").append(
      `<div class="body-content"><ul class="body-list"><li class="body-title">${data.tasks[0].task_title}</li></ul></div>`
    );

    data.tasks[0].assets.forEach((element) => {
      $(".body-list").append(
        `<li class="body-item">${element.asset_title}</li>`
      );
    });
  };

  const removeDataPopulate = () => {
    $(".body-content").remove();
  };

  $("#btn-open-journey-board").click(function () {
    if (isOpen) {
      $(".journey-board").css("transform", "translateX(-70%)");
      $("#btn-open-journey-board").attr("src", "assests/open.png");
      setTimeout(() => {
      ...
      $(".journey-board").css("transform", "translateX(0%)");
      $("#btn-open-journey-board").attr("src", "assests/close.png");
...
$(document).ready(function () {
  $(".asset-div-title .asset-title").each(function (i, el) {
    if (data.tasks[0].assets[i]) {
      $(el).text(data.tasks[0].assets[i].asset_title);
    }
  });

  $(".asset-description").each(function (i, el) {
    if (data.tasks[0].assets[i]) {
      $(el).html(
        `<span style="font-weight:bold">Description: </span> ${data.tasks[0].assets[i].asset_description}`
      );
    }
  });

  $(".asset-1-video").each(function (i, el) {
    if (data.tasks[0].assets[i]) {
      $(el).attr("src", data.tasks[0].assets[i].asset_content);
    }
  });
});
